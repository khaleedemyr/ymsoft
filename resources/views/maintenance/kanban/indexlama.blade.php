...
@section('content')
    ...
    <!-- Include Evidence modal -->
    @include('maintenance.kanban.modals.capture-evidence')
@endsection
...
<script src="{{ asset('build/js/maintenance-comment.js') }}"></script>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var drake = dragula([
                ...
                if (taskId && newStatus) {
                    // Special case: jika memindahkan dari PR ke PO, periksa status PR terlebih dahulu
                    if (newStatus === 'PO' && el.getAttribute('data-current-status') === 'PR') {
                        // Tampilkan loading state
                        Swal.fire({
                            title: 'Memeriksa...',
                            text: 'Memeriksa status PR',
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.success) {
                                    ...
                                    // Jika masih ada PR dengan status draft, tampilkan peringatan
                                    if (stats.draft > 0) {
                                        Swal.fire({
                                            icon: 'warning',
                                            title: 'Tidak dapat memindahkan task',
                                            html: `Masih ada ${stats.draft} PR dengan status Draft.<br>Semua PR harus disetujui atau ditolak terlebih dahulu.`,
                                            confirmButtonText: 'Mengerti'
                                        });
                                        
                                        // Batalkan perpindahan dengan memuat ulang halaman
                                        setTimeout(() => {
                                    ...
                                    // Jika tidak ada PR sama sekali, tampilkan peringatan
                                    if (stats.total === 0) {
                                        Swal.fire({
                                            icon: 'warning',
                                            title: 'Tidak dapat memindahkan task',
                                            text: 'Belum ada PR yang dibuat untuk task ini.',
                                            confirmButtonText: 'Mengerti'
                                        });
                                    ...
                                    // Jika semua PR sudah diproses, lanjutkan memindahkan task
                                    updateTaskStatus(taskId, newStatus);
                                } else {
                                    Swal.fire({
                                        icon: 'error',
                                        title: 'Error!',
                                        text: 'Gagal memeriksa status PR. Silahkan coba lagi.'
                                    });
                            ...
                            error: function(xhr) {
                                console.error('Error checking PR status:', xhr);
                                Swal.fire({
                                    icon: 'error',
                                    title: 'Error!',
                                    text: 'Gagal memeriksa status PR. Silahkan coba lagi.'
                        ...
                        // Untuk semua kasus lainnya, langsung update status
                        updateTaskStatus(taskId, newStatus);
                    }
                }
            });
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        response.forEach(function(task) {
                            let taskHtml = createTaskCard(task, task.board_id);
            ...
            function createTaskCard(task, boardId) {
                ...
                // Buat button untuk capture evidence, hanya tampilkan jika status task adalah 'IN_REVIEW'
                let captureEvidenceButton = task.status === 'IN_REVIEW' ? `
                    <button class="btn btn-sm btn-outline-success simple-evidence-btn me-2" 
                            data-task-id="${task.id}" 
        ...
        function loadTaskTimeline(taskId) {
            ...
            $.ajax({
                                ...
                                <div class="card-body p-3">
                                    ...
                                    <div class="d-flex align-items-center flex-wrap mb-2">
                                        <div class="me-4">
                                            ...
                                            <span class="fw-medium">${taskData.task_number}</span>
                                        </div>
                                        <div>
                ...
                error: function(xhr, status, error) {
                    ...
                    $('#timeline-due-date').html('<div class="alert alert-danger">Gagal memuat data task.</div>');
                    loadTaskTimelineData(taskId);
                }
            });
        }
        ...
        $(document).ready(function() {                    
            ...
            // Ambil data evidence untuk semua task yang ada
            loadTaskEvidence();
        });
        ...
        function addEvidenceToTask(taskId, evidenceData) {
            ...
            evidenceData.forEach((evidence, index) => {
                ...
                if (evidence.videos && evidence.videos.length > 0) {
                    // Maksimal tampil 2 video
                    const maxVideoDisplay = Math.min(2, evidence.videos.length);
                    let videosThumbnailHTML = '';
                    
                    // Buat HTML thumbnail video
                    for (let i = 0; i < maxVideoDisplay; i++) {
                        ...
                        videosThumbnailHTML += `
                            <div class="video-preview-item" data-media-path="${video.full_url || '/storage/' + video.path}" data-media-type="video">
                                <div class="video-thumbnail rounded" style="width: 60px; height: 40px; background: #000; position: relative; cursor: pointer;">
                                    <i class="ri-play-circle-line position-absolute" style="top: 50%; left: 50%; transform: translate(-50%, -50%); font-size: 24px; color: white;"></i>
                                </div>
                            </div>
                        `;
                    ...
                    // Jika video lebih dari 2, tambahkan badge +N
                    if (evidence.videos.length > 2) {
                        videosThumbnailHTML += `
                            <div class="video-preview-more">
                                <span class="badge rounded-pill bg-secondary">+${evidence.videos.length - 2}</span>
                            </div>
                        `;
                    }
        ...
        // Event untuk melihat foto/video evidence
        $(document).on('click', '.photo-preview-item, .video-preview-item', function() {
        ...
        function openMediaModal(mediaPath, mediaType) {
            // Buat modal untuk preview media
            const modalHTML = `
                    ...
                    <div class="modal-dialog modal-lg modal-dialog-centered">
                        <div class="modal-content">
                            ...
                            <div class="modal-body text-center">
                                ${mediaType === 'image' 
                                    ? `<img src="${mediaPath}" class="img-fluid rounded" style="max-height: 80vh;">` 
                                    : `<video src="${mediaPath}" controls autoplay class="w-100 rounded"></video>`
                                }
                            </div>
                            <div class="modal-footer">
                                <a href="${mediaPath}" class="btn btn-primary" download target="_blank">
                                    <i class="ri-download-line me-1"></i> Download
                                </a>
    ...
    <script>
        ...
        let evidenceIsVideoMode = false;
        let evidenceMediaRecorder = null;
        let evidenceRecordedChunks = [];
        let evidenceRecordingTimer = null;
        let evidenceRecordingSeconds = 0;
        let evidenceFacingMode = "environment"; // Default: kamera belakang
        ...
        function setupEvidenceEventHandlers() {
            // Event untuk tombol evidence di task card
            document.addEventListener('click', function(e) {
                if (e.target.closest('.simple-evidence-btn')) {
                    const btn = e.target.closest('.simple-evidence-btn');
                    evidenceTaskId = btn.getAttribute('data-task-id');
                    
                    // Log untuk debugging
                    console.log('Evidence button clicked for task:', evidenceTaskId);
            ...
            // Tombol ambil foto
            document.getElementById('evidenceTakePictureBtn').addEventListener('click', function() {
                console.log('Take picture button clicked');
                evidenceIsVideoMode = false;
                startEvidenceCamera();
            });
            
            // Tombol rekam video
            document.getElementById('evidenceRecordVideoBtn').addEventListener('click', function() {
                console.log('Record video button clicked');
                evidenceIsVideoMode = true;
                startEvidenceCamera();
            });
            
            // Tombol capture
            document.getElementById('evidenceCaptureBtn').addEventListener('click', function() {
                console.log('Capture button clicked');
                captureEvidencePhoto();
            });
            
            // Tombol mulai rekam
            document.getElementById('evidenceStartRecordingBtn').addEventListener('click', function() {
                console.log('Start recording button clicked');
                startEvidenceRecording();
            });
            
            // Tombol stop rekam
            document.getElementById('evidenceStopRecordingBtn').addEventListener('click', function() {
                console.log('Stop recording button clicked');
                stopEvidenceRecording();
            });
            
            // Tombol tutup kamera
            document.getElementById('evidenceCloseCameraBtn').addEventListener('click', function() {
                console.log('Close camera button clicked');
                stopEvidenceCamera();
            });
            
            // Tombol switch kamera
            document.getElementById('evidenceSwitchCameraBtn').addEventListener('click', function() {
                console.log('Switch camera button clicked');
                switchEvidenceCamera();
            });
            
            // Tombol save evidence
            document.getElementById('evidenceSaveBtn').addEventListener('click', function() {
                console.log('Save evidence button clicked');
                saveEvidence();
            });
        ...
        function startEvidenceCamera() {
            console.log('Starting camera, video mode:', evidenceIsVideoMode);
            
            // Stop kamera sebelumnya jika ada
            stopEvidenceCamera();
            
            // Update UI
            document.getElementById('evidenceCameraContainer').classList.remove('d-none');
            document.getElementById('evidenceRecordingIndicator').classList.add('d-none');
            
            if (evidenceIsVideoMode) {
                document.getElementById('evidenceCaptureBtn').classList.add('d-none');
                document.getElementById('evidenceStartRecordingBtn').classList.remove('d-none');
                document.getElementById('evidenceStopRecordingBtn').classList.add('d-none');
            } else {
                document.getElementById('evidenceCaptureBtn').classList.remove('d-none');
                document.getElementById('evidenceStartRecordingBtn').classList.add('d-none');
                document.getElementById('evidenceStopRecordingBtn').classList.add('d-none');
            }
            
            // Constraints untuk getUserMedia
            const constraints = {
                audio: evidenceIsVideoMode,
                video: {
                    width: { ideal: 1280 },
                    height: { ideal: 720 },
            ...
            navigator.mediaDevices.getUserMedia(constraints)
                .then(function(stream) {
                    ...
                    const videoElement = document.getElementById('evidenceCameraView');
                    videoElement.srcObject = stream;
                    videoElement.play();
                })
                .catch(function(err) {
                    console.error('Error accessing camera:', err);
                    document.getElementById('evidenceBrowserWarning').classList.remove('d-none');
                    document.getElementById('evidenceCameraContainer').classList.add('d-none');
                    
                    Swal.fire({
                        icon: 'error',
                        title: 'Akses Kamera Gagal',
                        text: 'Browser tidak dapat mengakses kamera. Pastikan browser mendukung dan Anda memberikan izin kamera.',
                        confirmButtonText: 'Mengerti'
                    });
                });
        }
        ...
        function captureEvidencePhoto() {
            ...
            try {
                ...
                // Tambahkan penanganan khusus jika video belum siap
                if (!videoElement || !videoElement.videoWidth) {
                    console.log('Video not ready yet, waiting...');
                    
                    // Tambahkan class loading untuk menandakan sedang mempersiapkan
                    document.getElementById('evidenceCaptureBtn').classList.add('disabled');
                    document.getElementById('evidenceCaptureBtn').innerHTML = '<span class="spinner-border spinner-border-sm me-1"></span> Menyiapkan...';
                    
                    // Coba lagi dalam 500ms
                    setTimeout(function() {
                        document.getElementById('evidenceCaptureBtn').classList.remove('disabled');
                        document.getElementById('evidenceCaptureBtn').innerHTML = '<i class="ri-camera-line me-1"></i> Capture';
                        captureEvidencePhoto();
                    }, 500);
                    return;
                }
                ...
                // Set ukuran canvas sesuai dengan video atau gunakan ukuran standar
                ...
                if (!context) {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: 'Gagal mendapatkan canvas context'
                    });
                    return;
                }
                ...
                try {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: 'Gagal memproses data gambar: ' + error.message
                    });
                }
            } catch (error) {
                console.error('Error capturing photo:', error);
        ...
        // Update preview dengan cara sederhana menggunakan dataUrl
        function updateEvidencePreviewSimple() {
            console.log('Updating preview with simple method');
            const container = document.getElementById('evidencePreviewContainer');
            container.innerHTML = '';
            
            // Generate HTML untuk foto dari dataUrl
            evidencePhotos.forEach(function(photo, index) {
                const photoHtml = `
                    <div class="position-relative" style="width: 120px; height: 120px; margin-right: 8px; margin-bottom: 8px;">
                        <img src="${photo.dataUrl || photo.url}" class="img-thumbnail" style="width: 100%; height: 100%; object-fit: cover;">
                        <button type="button" class="btn btn-sm btn-danger position-absolute top-0 end-0 delete-media-btn" 
                                data-type="photo" data-index="${index}">
                            <i class="ri-delete-bin-line"></i>
                        </button>
            ...
            // Generate HTML untuk video dengan format baru
            evidenceVideos.forEach(function(video, index) {
                // Validasi URL tersedia
                if (!video.url) {
                    console.error('Video URL not available for index', index);
                    return;
                }
        ...
        // Fungsi untuk simpan evidence yang dimodifikasi menggunakan dataUrl
        function saveEvidence() {
            console.log('Saving evidence for task:', evidenceTaskId);
            ...
            if (evidencePhotos.length === 0 && evidenceVideos.length === 0) {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Silakan ambil minimal 1 foto atau video terlebih dahulu'
                });
                return;
            }
            ...
            formData.append('notes', document.getElementById('evidenceNotes').value);
            formData.append('_token', document.querySelector('meta[name="csrf-token"]').getAttribute('content'));
            
            // Log process
            console.log('FormData preparation, task_id:', evidenceTaskId);
            console.log('Photos count:', evidencePhotos.length);
            console.log('Videos count:', evidenceVideos.length);
            
            // Tambahkan foto ke form data dari dataUrl
            evidencePhotos.forEach(function(photo, index) {
                console.log('Adding photo to form data:', index);
                
                // Data URL atau Blob, pastikan ada satu yang tersedia
                let photoBlob;
                if (photo.dataUrl) {
                    console.log('Photo has dataUrl, converting to blob');
                    photoBlob = dataURLtoBlob(photo.dataUrl);
                    formData.append(`photos[${index}]`, photoBlob, `photo_${index}.jpg`);
                } else if (photo.blob) {
                    console.log('Photo has blob directly');
                    formData.append(`photos[${index}]`, photo.blob, `photo_${index}.jpg`);
                }
            });
            ...
            evidenceVideos.forEach(function(video, index) {
                console.log('Adding video to form data:', index);
                if (video.blob) {
                    // Pastikan ukuran blob yang valid
                    console.log('Video blob size:', video.blob.size);
                    formData.append(`videos[${index}]`, video.blob, `video_${index}.webm`);
                } else {
                    console.error('Video blob not available for index', index);
                }
            });
            
            // Log request details untuk debugging
            console.log('FormData prepared with:');
            console.log('- task_id:', evidenceTaskId);
            console.log('- notes length:', document.getElementById('evidenceNotes').value.length);
            console.log('- photos count:', evidencePhotos.length);
            console.log('- videos count:', evidenceVideos.length);
            ...
            // Kirim ke server dengan fetch API
            fetch('/maintenance/kanban/upload-evidence', {
                method: 'POST',
                body: formData
            })
            .then(response => {
            ...
            .catch(error => {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Terjadi kesalahan saat menyimpan evidence: ' + error.message
                });
            });
        }
        
        // Helper function untuk convert dataURL ke Blob
        ...
        function startEvidenceRecording() {
            ...
            evidenceRecordedChunks = [];
            
            // Try to initialize MediaRecorder
            try {
                let options = { mimeType: 'video/webm;codecs=vp9,opus' };
                
                try {
                    evidenceMediaRecorder = new MediaRecorder(evidenceStream, options);
                    console.log('MediaRecorder initialized with options:', options);
                } catch (e) {
                    console.warn('MediaRecorder with vp9 not supported, trying fallback', e);
                    
                    options = { mimeType: 'video/webm' };
                    evidenceMediaRecorder = new MediaRecorder(evidenceStream, options);
                    console.log('MediaRecorder initialized with fallback options:', options);
                }
                
                // UI updates
                document.getElementById('evidenceStartRecordingBtn').classList.add('d-none');
                document.getElementById('evidenceStopRecordingBtn').classList.remove('d-none');
                document.getElementById('evidenceRecordingIndicator').classList.remove('d-none');
                
                // Start recording timer
                startEvidenceRecordingTimer();
                
                // Gather video data
                evidenceMediaRecorder.ondataavailable = function(e) {
                    if (e.data && e.data.size > 0) {
                        console.log('Video data available, chunk size:', e.data.size);
                        evidenceRecordedChunks.push(e.data);
                    }
                };
                
                // When recording stops
                evidenceMediaRecorder.onstop = function() {
                    console.log('Recording stopped, chunks:', evidenceRecordedChunks.length);
                    stopEvidenceRecordingTimer();
                    
                    // Create video blob
                    if (evidenceRecordedChunks.length > 0) {
                        const blob = new Blob(evidenceRecordedChunks, { type: 'video/webm' });
                ...
                // Start recording
                evidenceMediaRecorder.start(1000); // Collect data every second
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Gagal memulai perekaman video: ' + error.message
                });
            }
        }
        
        // Fungsi untuk stop rekaman
        function stopEvidenceRecording() {
            console.log('Stopping video recording');
            
            if (evidenceMediaRecorder && evidenceMediaRecorder.state !== 'inactive') {
                evidenceMediaRecorder.stop();
            }
        }
        
        // Fungsi untuk memulai timer rekaman
        function startEvidenceRecordingTimer() {
            evidenceRecordingSeconds = 0;
            updateEvidenceRecordingTime();
            
            evidenceRecordingTimer = setInterval(function() {
                evidenceRecordingSeconds++;
                updateEvidenceRecordingTime();
            }, 1000);
        }
        
        // Fungsi untuk menghentikan timer
        function stopEvidenceRecordingTimer() {
            if (evidenceRecordingTimer) {
                clearInterval(evidenceRecordingTimer);
                evidenceRecordingTimer = null;
            }
        }
        
        // Fungsi untuk update display timer
        function updateEvidenceRecordingTime() {
            const minutes = Math.floor(evidenceRecordingSeconds / 60);
            const seconds = evidenceRecordingSeconds % 60;
            
            const display = 
                (minutes < 10 ? '0' : '') + minutes + ':' +
                (seconds < 10 ? '0' : '') + seconds;
                
            document.getElementById('evidenceRecordingTime').textContent = display;
        }
        
        // Fungsi untuk switch kamera
        function switchEvidenceCamera() {
            console.log('Switching camera from', evidenceFacingMode, 'to', evidenceFacingMode === 'environment' ? 'user' : 'environment');
        ...
        function resetEvidenceForm() {
            console.log('Resetting evidence form');
            
            // Stop camera
            stopEvidenceCamera();
            
            // Revoke all object URLs
            evidencePhotos.forEach(function(photo) {
                if (photo.url) {
                    URL.revokeObjectURL(photo.url);
                }
            });
            
            evidenceVideos.forEach(function(video) {
                if (video.url) {
            ...
            // Reset UI
            document.getElementById('evidencePreviewContainer').innerHTML = '<div class="text-muted" id="evidenceNoMediaMessage">Belum ada media yang ditangkap</div>';
            document.getElementById('evidenceNotes').value = '';
            document.getElementById('evidenceBrowserWarning').classList.add('d-none');
        }
    </script>
